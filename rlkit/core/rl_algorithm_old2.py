...
class MetaRLAlgorithm(metaclass=abc.ABCMeta):
    ...
    def load_model(self):
        """
        Load pretrained models if pretrained_path is not empty
        :return:
        """
    ...
    def collect_paths(self, idx, epoch, run):
        ...
        while num_transitions < self.num_steps_per_eval:
            ...
            if num_trajs >= self.num_exp_traj_eval:
                self.agent.infer_posterior(self.agent.context, task_indices=np.array([idx]))
    ...
    def evaluate(self, epoch):
        ...
        for idx in indices:
            ...
            for _ in range(self.num_steps_per_eval // self.max_path_length):
                ...
                self.agent.infer_posterior(context, task_indices=np.array([idx]))
...
class OfflineMetaRLAlgorithm(MetaRLAlgorithm):
    def __init__(
            self,
            env,
            agent,
            ...
            train_epoch=3000,
            eval_epoch=3000,
            sample=True, # whether use stochastically sampled trajectories for training
            n_trj=5, # number of trajectories of each epoch to be used for training
            **kwargs
    ):
        super().__init__(env, agent, train_tasks, eval_tasks, **kwargs)
        # separate replay buffer for
        # storing trajectories of all training/evaluation tasks
        self.train_buffer = MultiTaskReplayBuffer(
            self.replay_buffer_size,
            env,
            self.train_tasks,
        )
        self.eval_buffer = MultiTaskReplayBuffer(
            self.replay_buffer_size,
            env,
            self.eval_tasks,
        )
    ...
    def test(self):
        '''
        load pretrained models and perform meta-testing only
        :return:
        '''
    ...
    def train(self):
        ...
        for it_ in gt.timed_for(
            ...
            # for train_step in range(1):
                indices = np.random.choice(self.train_tasks, self.meta_batch)
                self._do_training(indices, zloss=True)
                self._n_train_steps_total += 1
            gt.stamp('train')
    ...
    def collect_paths(self, idx, epoch, run, buffer):
        ...
        # num_trajs = 0
        while num_transitions < self.num_steps_per_eval:
            path, num = self.offline_sampler.obtain_samples(buffer=buffer,
                                                            deterministic=self.eval_deterministic, max_samples=self.num_steps_per_eval - num_transitions, max_trajs=1, accum_context=True, rollout=True)
            paths += path
            num_transitions += num
            # num_trajs += 1
            # if num_trajs >= self.num_exp_traj_eval:
