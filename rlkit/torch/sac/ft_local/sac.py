...
class FOCALSoftActorCritic(OfflineMetaRLAlgorithm):
    ...
    def _optimize_c(self, indices, context):
        ...
        if self.use_information_bottleneck:
            ...
            policy_outputs, task_z, task_z_vars = self.agent(obs, context, idx=indices[0])
        new_actions, policy_mean, policy_log_std, log_pi = policy_outputs[:4]
    ...
    def z_loss(self, indices, task_z, task_z_s, b, epsilon=1e-8, var_epsilon=0.1):
        '''
        ...
        for i in range(len(indices)):
            for j in range(i+1, len(indices)):
                ...
                if indices[i] == indices[j]:
                    if pos_cnt == 0:
                        pos_z_loss.append(2 * torch.mean((task_z[idx_i] - task_z[idx_j]) ** 2/(task_z_s[idx_i] ** 2 + task_z_s[idx_j] ** 2 + var_epsilon)))
                    else:
                        pos_z_loss[0] += 2 * torch.mean((task_z[idx_i] - task_z[idx_j]) ** 2/(task_z_s[idx_i] ** 2 + task_z_s[idx_j] ** 2 + var_epsilon))
                    pos_cnt += 1
                else:
                    if neg_cnt == 0:
                        neg_z_loss.append(- 2 * torch.mean((task_z[idx_i] - task_z[idx_j]) ** 2/(task_z_s[idx_i] ** 2 + task_z_s[idx_j] ** 2 + var_epsilon)))
                    else:
                        neg_z_loss[0] -= 2 * torch.mean((task_z[idx_i] - task_z[idx_j]) ** 2/(task_z_s[idx_i] ** 2 + task_z_s[idx_j] ** 2 + var_epsilon))
                    neg_cnt += 1

        return ((pos_z_loss[0] if len(pos_z_loss) else 0)/(pos_cnt + epsilon) + (neg_z_loss[0] if len(neg_z_loss) else 0)/(neg_cnt + epsilon)).to(ptu.device)
    ...
    def _take_step(self, indices, context):
        ...
        if self.use_information_bottleneck:
            ...
            policy_outputs, task_z, task_z_s = self.agent(obs, context, idx=indices[0])
        ...
        if not self.use_information_bottleneck:
            z_loss = self.z_loss_weight * self.z_loss(indices=indices, task_z=task_z, task_z_s=task_z_s, b=b)
            z_loss.backward(retain_graph=True)

        task_z = utils.clip_v2(task_z, -10., 10.)
        new_actions, policy_mean, policy_log_std, log_pi = policy_outputs[:4]
