...
class PEARLAgent(nn.Module):
    ...
    def infer_posterior(self, context):
        ...
        # sum rather than product of gaussians structure
            ...
            self.z_s = torch.std(params, dim=1) / torch.sqrt(params.size()[1])
        self.sample_z()
    ...
    def forward(self, obs, context, idx=0):
        ...
        if not self.use_ib:
            task_z_s = [z.repeat(b, 1) for z in self.z_s]
            task_z_s = torch.cat(task_z_s, dim=0)
            return policy_outputs, task_z, task_z_s
    ...
    def log_diagnostics(self, eval_statistics):
        ...
        eval_statistics['task_idx'] = self.task_idx
